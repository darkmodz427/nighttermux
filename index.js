...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'FN:ZEUS\n' //NAMA LU
            + 'ORG:Owner Bot;\n' // NAMA CONTACT OWNER
            + 'TEL;type=CELL;type=VOICE;waid=5521980815830:+55 69 9389-9391\n' // NOMER OWNER
            + 'END:VCARD'
prefix = '#'//PREFIX BOT
blocked = ['5521980815830']//NOMER BLOCK
limitawal = '50'
cr = '*VINIZEUS E HYOUKA DOMINA KRLH*'
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Flw Otário @${num.split('@')[0]}, Não volte por favor`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["5521980815830@s.whatsapp.net"] // Nomer Owner
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '5521980815830@s.whatsapp.net'//Nomer Owner
                        const isEventon = isGroup ? event.includes(from) : false
                        const isRegister = checkRegisteredUser(sender)
                        pushname = client.contacts[sender] != undefined ? client.contacts[sender].vname || client.contacts[sender].notify : undefined

                        // Options
                        const botName = 'NightBot'//Nama Bot
                        const ownerName = 'Vinizeus E Hyouka'//Nama Owner
            ...
            try {
                ...
                if (requiredXp <= getLevelingXp(sender)) {
                    ...
                    await reply(`*「 PASSOU DE LEVEL MENOR 😎 」*\n\n➸ *Nome*: ${sender}\n➸ *XP*: ${getLevelingXp(sender)}\n➸ *Level*: ${getLevel} -> ${getLevelingLevel(sender)}\n\nParabéns!! 🎉🎉`)
                }
            } catch (err) {
                console.error(err)
            }
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
                ...
                case 'gay':
					...
					const ab =['10%', '15%', '20%', '32%', '34%', '39%', '41%', '49%', '50%', '53%', '55%', '61%', '67%', '69%', '74%', '75%', '80%', '88%', '90%', '93%', '99%', '100%', '200%']
